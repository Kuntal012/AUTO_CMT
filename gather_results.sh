#/bigdata/aghoshlab/kchau012/Cascadia_VlFE/AUTO/Test1/March14/20140308/12

echo "GP  COMB  YEAR MONTH DAY HR MIN SECS LONG LAT DEP MAG VAR CLVD Var1 Var2 Var3"
for pp in `ls -d GP*`
do
cd $pp
...
do
  ...
  s3_c=`echo $s3'>0' | bc -l`
  s4_c=`echo $s4'>0' | bc -l`
  #for checking if VAR is greater than CLVD
  s_c_vr=`echo $var'>'$clvd| bc -l`
  #echo "$pp  $i  $day  $tim  $lon  $lat  $dep  $mag  $var  $clvd $s1 $s2 $s3" 
  if [[ $s1_c -eq 1 && ${s2_c} -eq 1 && ${s3_c} -eq 1 && ${s4_c} -eq 1 && ${s_c_vr} -eq 1 ]]
  then
  	echo "$pp  $i  $day  $tim  $lon  $lat  $dep  $mag  $var  $clvd $s1 $s2 $s3 $s4"
  fi
