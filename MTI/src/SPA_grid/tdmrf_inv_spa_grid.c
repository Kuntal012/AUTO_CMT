  ...
  for(gfnum=0 ; gfnum < deplabn ; gfnum++){
	...
	for(j=z,k=0 ; j<z+np; j++,k++){
	  ...
	  if( waved.cmpn[i] == 3 ){
	    ...
	    // Transverse
	    strc.mtxA[cnt1][0+5*n] = 0.5*sin(2.0*azimuth[i])* grenf.gfnc[i][0][j-tsp*n] ;    // T Mxx
	    strc.mtxA[cnt1][1+5*n] = (-0.5)*sin(2.0*azimuth[i])* grenf.gfnc[i][0][j-tsp*n];  // T Myy
	    strc.mtxA[cnt1][2+5*n] = (-1.0)*cos(2.0*azimuth[i])* grenf.gfnc[i][0][j-tsp*n];  // T Mxy
	    strc.mtxA[cnt1][3+5*n] = (-1.0)*sin(azimuth[i])*grenf.gfnc[i][1][j-tsp*n] ;      // T Mxz
	    strc.mtxA[cnt1][4+5*n] =        cos(azimuth[i])*grenf.gfnc[i][1][j-tsp*n] ;      // T Myz
	    
	    // Radial
	    strc.mtxA[cnt2][0+5*n] = 0.5*(grenf.gfnc[i][4][j-tsp*n] 
					  - cos(2.0*azimuth[i])*grenf.gfnc[i][2][j-tsp*n]) ; // R Mxx
	    strc.mtxA[cnt2][1+5*n] = 0.5*(grenf.gfnc[i][4][j-tsp*n] 
					  + cos(2.0*azimuth[i])*grenf.gfnc[i][2][j-tsp*n]) ; // R Myy
	    strc.mtxA[cnt2][2+5*n] = (-1.0)*sin(2.0*azimuth[i])* grenf.gfnc[i][2][j-tsp*n];  // R Mxy
	    strc.mtxA[cnt2][3+5*n] =        cos(azimuth[i])*grenf.gfnc[i][3][j-tsp*n] ;      // R Mxz
	    strc.mtxA[cnt2][4+5*n] =        sin(azimuth[i])*grenf.gfnc[i][3][j-tsp*n] ;      // R Myz
	    
	    if( waved.cmpn[i] == 3 ){
	      // Zcomp
	      strc.mtxA[cnt3][0+5*n] = 0.5*(grenf.gfnc[i][7][j-tsp*n] 
					    - cos(2.0*azimuth[i])*grenf.gfnc[i][5][j-tsp*n]) ; // Z Mxx
	      strc.mtxA[cnt3][1+5*n] = 0.5*(grenf.gfnc[i][7][j-tsp*n] 
					    + cos(2.0*azimuth[i])*grenf.gfnc[i][5][j-tsp*n]) ; // Z Myy
	      strc.mtxA[cnt3][2+5*n] = (-1.0)*sin(2.0*azimuth[i])* grenf.gfnc[i][5][j-tsp*n];  // Z Mxy
	      strc.mtxA[cnt3][3+5*n] =        cos(azimuth[i])*grenf.gfnc[i][6][j-tsp*n] ;      // Z Mxz
	      strc.mtxA[cnt3][4+5*n] =        sin(azimuth[i])*grenf.gfnc[i][6][j-tsp*n] ;      // Z Myz
	    }
	  }
	  //weight for distance
	...
	if( mmmin > fabs(mm[i])) 
      ...
      fprintf( fpout, "%s %s %s %s %s %d %s %s %s ",
	       evlab[0],evlab[1],evlab[2],evlab[3],evlab[4],z,evlab[5],evlab[6],deps[gfnum]) ;
