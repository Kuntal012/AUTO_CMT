...
####
# Getting the parameters from 'prm_tdmrf_inv'--AG
open(IN,$prmf) ;
while(<IN>){
    if($_ =~ /DTIME/){
	chop($_) ;
...
#    print "$depths[$i]\n" ;
#}
# Getting station names, and locations(lat-lon)--AG
open(IN,$stlist) ;
while(<IN>){
    chop($_) ;
    @data=split(/\s+/,$_) ;
...
###############################################################
##############
# Loop over origin time and depth--AG
$oo=0 ; $dd=0 ;
...
for( $oo=$btime ; $oo<=$btime+$maxtime ; $oo++){
	...
	printf OUT "%d %03d 1 1\n",$#stats+1,$depths[$dd] ;
        ...
        # Looping over stations to make an input file for 'src/tdmrf_inv'--AG
	for($i=0 ; $i<=$#stats; $i++){
	    $stcd=$stats[$i] ;
	...
	for($i=0 ; $i<=$#stats ; $i++){
	    ...
	    $greenfile=sprintf("%s/cs%03dd%03d",$gfpath,$sdelta{$stcd},$depths[$dd]) ;
	    printf OUT "%s 0 %d\n",
	    $greenfile,$twind ;
	}
	...
	print OUT "0\n" ;
 	close(OUT) ;
# Done writting the input file 'tdmrf_inv.in_????_???'--AG

        # run 'src/tdmrf_inv' to calculate the moment tensor that best fit the data for this event location--AG
	system "$TDMRF tdmrf_inv.in_$itrlabel > /dev/null" ;
#	system "$TDMRF tdmrf_inv.in_$itrlabel" ;
        # makes an output file 'tdmrf_inv.out'--AG
	...
	$onsets,$depths[$dd],$str,$dip,$rak,$clvd,$moment,$varred ;
        # some output parameters get written and collected in an output file 'itr_res_inv.out'--AG
