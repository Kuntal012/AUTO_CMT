...
for($i=0; $i<=$#ARGV; $i++){
	...
	printf "tdmrf_gmt_out_ah_forML.pl [input_parameter_file]\n",
...
if( -e "itr_res_inv.out.all" ){
    ...
    $timelabel=$line[2] ;
    open(CMD,"$gmtdir/minmax -C itr_res_inv.out.all|");
    $line=<CMD> ;
    chop($line) ;
    close(CMD) ;
    @lines=split(/\s+/,$line) ;
...
for( $pagen=0, $pagei=1 ; $pagen<$stnum ; $pagen+=$pagewave, $pagei++){
    ...
    system ( "$gmtdir/psbasemap -R0/19/0/28 -Jx1c/-1c  -G255 -P -K -X1 -Y1  > $outps" ) ;
...
############ make cpt #####
    ...
    system"$gmtdir/psscale -B10 -C./residual_cpt.tmp -D2/0.5/3/0.2h -O -P -K >>$outps" ;
###########################
