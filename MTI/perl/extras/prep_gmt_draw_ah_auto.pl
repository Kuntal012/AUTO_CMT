...
####
$S2H="$MTIDIR"."perl/bat_sac2helm_ah_auto.pl" ;
$SINV="$MTIDIR"."perl/sub_tdmrf_inv_cmt_ah_auto.pl" ;
$TGMT="$MTIDIR"."perl/tdmrf_gmt_out_ah_auto.pl" ;
####

$fhead=$ARGV[0] ;
$ddir=$ARGV[1] ;

open(IN, "TW_flags.list" )  ;
$line=<IN> ;
chop($line) ;
@data=split(/\s+/,$line) ;
$YYYY=$data[0]  ;
...
# make sure than in_prm file is correct

open(IN,"grid_inv_ah.prm") || die "Grid_inv.prm not found\n" ;
