...
### ARGV[6] mm
$mm=$ARGV[6] ;
### ARGV[7] # Data directory
$ddir=$ARGV[7] ;
####################
#$MTIDIR="/disk2/yito/MTI_pgc" ;
#$MTIDIR="/home/yito/MTI_pgc" ;
$MTIDIR="/home/ehuesca/VLF/MTI" ;
$S2H=$MTIDIR."/perl/bat_sac2helm_ag.pl" ;
$MGI=$MTIDIR."/perl/make_grid_in_ag_forML.pl" ;
$GRD=$MTIDIR."/src/SPA_grid/tdmrf_inv_spa_grid";
#$ddir="/home/ehuesca/VLF/raw_data/August10/20100819/hh/zp/" ;
####################
open(IN,$prmfile) ;
while(<IN>){
    chop($_) ;
...
for($lon=$lons[0] ;$lon<=$lons[1] ;$lon+=$ddeg){
	...
	print "$S2H $filehead cmt_stat.list $lon $lat $ddir\n" ;
	system "$S2H $filehead cmt_stat.list $lon $lat $ddir" ;
