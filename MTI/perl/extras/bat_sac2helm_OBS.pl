...
while(<IN>){
    ...
    for($i=1 ; $i<=$#data ; $i++){
	if($data[$i] =~ /1$/){
	    $target1=$ddir.$filehead."$data[0]".".."."$data[$i]".".D.SAC_deconv" ;
	#}elsif($data[$i] =~ /E$/){
          #  $targetE=$ddir.$filehead."$data[0]".".."."$data[$i]".".D.SAC_deconv" ;
	#}#elsif($data[$i] =~ /N$/){
	  #  $targetN=$ddir.$filehead."$data[0]".".."."$data[$i]".".D.SAC_deconv" ;
	}elsif($data[$i] =~ /2$/){
            $target2=$ddir.$filehead."$data[0]".".."."$data[$i]".".D.SAC_deconv" ;
	}elsif($data[$i] =~ /Z$/){
    ...
    print $exesac."\n" ;
    open(SAC,"|$exesac") ;
    print "read $target1 $target2\n" ;
    print SAC "read $target1 $target2\n" ;
