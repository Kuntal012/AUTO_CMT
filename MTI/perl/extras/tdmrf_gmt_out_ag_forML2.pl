...
open( IN, $inprmfile )  || die "$inprmfile not Fount " ;
$line=<IN> ;
chop($line ) ;
@data=split(/\s+/, $line ) ;
$stnum=$data[0] ;
...
### SOURCE TIME FUNCTION ##
    system ("psxy -R -Jx -M -O -P -K -W0.5p inputdata_gmt3_0.tmp >> $outps" ) ;
    system ("psxy -R -Jx -M -O -P -K -W1p inputdata_gmt3_1.tmp >> $outps" ) ;
# Moment tesnor #
    #system ("psmeca -R -Jx -Sm1.0 -CpenP0.03 -P -O -K -T -a0.1c/d -G110/110/110 inputdata_gmt4.tmp >> $outps" ) ; # commented out by AG
    system ("psmeca -R -Jx -Sm1.0 -P -O -K -T -Fa0.1c/d -G110/110/110 inputdata_gmt4.tmp >> $outps" ) ; # added by AG
#### OBS and SYN ####
    system("psxy -R -Jx -M -O -P -K -W1p inputdata_gmt_p$pagei.tmp >> $outps") ;
    system("psxy -R -Jx -M -O -P -K -W1t2_2:0p/255/0/0 inputdata_gmt2_p$pagei.tmp >> $outps") ;
    system("pstext -R -Jx -O -P -K -V  inputdata_gmts_p$pagei.tmp  >> $outps") ;
#### Parameter Label ##
    system("pstext -R -Jx -O -P -K -V  inputdata_gmt2_1.tmp  >> $outps") ;
    system("pstext -R -Jx -O -P -K -V  inputdata_gmt2_2.tmp  >> $outps") ;
    if( -e "itr_res_inv.out"){
	system("pstext -R -Jx -O -P -K -V inputdata_gmt9.tmp>>$outps");
    }
#### Plot Map ######
    system("psbasemap -R$mapropt -J$mapjopt -B$mapbopt -O -P -K -X9 -Y21.5 >> $outps ") ;
    system("pscoast -R -J$mapjopt -C200/255/255 -S200/255/255 -D$coastf -G200/255/200 -W0.5p -O -P -K >>$outps") ;
    system("psxy -R -J$mapjopt -Sa0.5 -G200/0/0 -O -P -K -W0.5p inputdata_gmt5.tmp>>$outps" ) ;
############ make cpt #####
    system"makecpt -Chot -T$sort_lvr[0]/$sort_lvr[$#sort_lvr]/1 -Z > residual_cpt.tmp" ;
    system"psscale -B10 -Cresidual_cpt.tmp -D2/0.5/3/0.2h -O -P -K >>$outps" ;
###########################
    system("psxy -R -J$mapjopt -Sc0.15 -Cresidual_cpt.tmp  -O -P -K -W0.5p inputdata_gmt6.tmp >>$outps") ;
    unless( -e "itr_res_inv.out" ){
	system("psxy -R -J$mapjopt -Ss0.15 -Cresidual_cpt.tmp -O -P -W0.5p inputdata_gmt7.tmp >>$outps") ;
    } else {
### plot Depth Var ####
	system("psxy -R -J$mapjopt -Ss0.15 -Cresidual_cpt.tmp -O -P -K -W0.5p inputdata_gmt7.tmp >>$outps") ;
	system("psbasemap -R$vrropt -JX3c/-4c -B$vrbopt -O -P -K -X6 -Y1 >>$outps");
	system("psmeca -R -JX -Sa0.20 -O -P inputdata_gmt8.tmp>>$outps") ;
    }
}
#####################
