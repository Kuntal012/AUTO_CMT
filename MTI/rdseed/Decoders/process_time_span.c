...

{
	...
	int i, j, now_where, num_items, default_Lrecl;

	default_Lrecl = LRECL;

	now_where = ftell(inputfile);

	fseek(inputfile, ((t74->start_index-1)*LRECL) - now_where, 1);
	...
	{ 
		...
		{ 
			if ((j == (t74->end_index-t74->start_index)) && (LRECL < default_Lrecl))
