...
/*  THIS IS ONLY A SKETCH, NOT WORKING CODE
	if (*blockette_length > current_blkt_len)
	{
		current_blkt_len = *blockette_length + BLOCKETTE_INCREMENT;
		if (blockette != NULL) free (blockette);
		blockette = (char *) malloc (current_blkt_len);
	}
*/
