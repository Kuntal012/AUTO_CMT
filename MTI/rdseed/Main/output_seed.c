...

{
	...
	while (type53_ptr)
	{
        ...
        memset(buff, 0, 46 + (48 * (type53_ptr->number_zeroes + type53_ptr->number_poles)) + 1);
		...
		sprintf(wrkstr, "%04d", strlen(buff));
    ...
    while (type54_ptr)
    {
		...
		buff = malloc(3 +	/* blockette type */
				...
				(24 * (type54_ptr->number_numerators+
                                	type54_ptr->number_denominators)) + 1); 
...

{
    ...
    while (type55_ptr)
    {
        ...
        buff = malloc(19 + /* type + size + seq # + in + out units + # resps */
					  (60 * type55_ptr->number_responses) + 1);

        if (buff == NULL)
            return 1;       /* error condition */

		memset(buff, 0, 19 + (60 * type55_ptr->number_responses) + 1);    
        /* --------------------------------------- */
 ...
 
{
    ...
    {
        /* allocate room */
        buff = malloc(19 + (24 * type56_ptr->number_corners) + 1);
 
        if (buff == NULL)
            return 1;       /* error condition */

		memset(buff, 0, 19 + (24 * type56_ptr->number_corners) + 1);
 ...
 
{
    ...
    while (type58_ptr)
    {
        /* allocate room for history values */
        buff = malloc(35 + (47 * type58_ptr->number_calibrations) + 1);

        if (buff == NULL)
            return 1;       /* error condition */

		memset(buff, 0, 35 + (47 * type58_ptr->number_calibrations) + 1);
...

{
    ...
    while (type61_ptr)
    {
        ...
        buff = malloc(9 + strlen(type61_ptr->name ? type61_ptr->name : "") + 
					  11 +
					 (14 * type61_ptr->number_numerators) + 1);

        if (buff == NULL)
            return 1;       /* error condition */

		memset(buff, 0, 
					  9 + strlen(type61_ptr->name ? type61_ptr->name : "") + 
                      11 +
					 (14 * type61_ptr->number_numerators) + 1);
        ...
        for (i = 0; i < type61_ptr->number_numerators; i++)
        {
            ...
            sprintf(ch_ptr, "%14.7e",
                type61_ptr->numerator[i]);
...

{
    ...
    while (type62_ptr)
    {
        ...
        buff = malloc(81 + (24 * type62_ptr->number_coefficients) + 1);

        if (buff == NULL)
            return 1;       /* error condition */

	memset(buff, 0, 81 + (24 * type62_ptr->number_coefficients) + 1);
...

{
	...
	if (spans_LRECL(fptr, 7))
	{
		...
		sprintf(wrkstr, "%06d%c*", (ftell(fptr) / LRECL) + 1, type);
...
void position_to_single_record_read(int *num_bytes_read, char *lrecord, FILE *fptr) {
	...
	if (lrecord[6] != 'D' && lrecord[6] != 'Q' && lrecord[6] != 'R')  {
		return;
	}
        if (hdr->bofb != 0) {
