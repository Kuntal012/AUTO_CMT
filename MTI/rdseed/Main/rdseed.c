...
/*	fprintf(stderr, "<< IRIS SEED Reader, Release %s.%1.1f >>\n", VERSION, PATCHLEVEL); */

	/* no patchlevel  */

	fprintf(stderr, "<< IRIS SEED Reader, Release %s  >>\n",VERSION);
...
begin:
	...
	if (data_flag||mini_flag) if ((seismic_data = (float *) malloc((seis_buffer_length+000)*4)) == NULL)
	{
	...
	{
/*
		if (options->next_arg == 0) numitems = 0;
		else
			for (i = options->next_arg-1, j = 0; i < argc; i++, j++)
			{
				item[j] = argv[i]; 
				numitems = j+1;
			}
*/
	...
	switch (output_format)
	{
		...
		case 5 : sprintf(outputformat, "sac.ascii");
			break;

		default: sprintf (outputformat, "unknown"); break; 
	}
	if (outputformat_flag)
		fprintf (stderr,"Output data format will be %s.\n", outputformat);		
	...
	while (more_data == TRUE)
	{
		...
		/*****************************************************/
			...
			if (num_bytes_read != 0 && (alt_headerfile!=NULL) && !reading_alt_file) {
				if (precord[6] == 'D' || precord[6] == 'Q' || precord[6] == 'R') {
...
skip_flag:
		...
		if (snoop_flag) fprintf(stderr,"%06ld ", input.recordnumber);
			...
			case 'Q':
				...
				if (times_flag)
				{
                                                        ...
                                                        chk_network(input_data_hdr->network))
                                        {
                                                ...
                                                if (input_data_hdr->time.year <
1950 ||                                          
                                                        input_data_hdr->time.year > 2010)                                        
                                                        swap_fsdh(&input_data_hdr, &input_data_ptr);
...
1950 ||                                          
				...
				if (read_summary_flag && (input_file_type == DISK_DEVICE))
				{
					if (input_file_type == DISK_DEVICE)
					{
				...
				{ 
							...
							chk_network(input_data_hdr->network))
					{
						...
						/*if (strcmp(prevStn, "") != 0)
						{
							if ((strncmp(prevStn, input_data_hdr->station, 5) != 0) ||
                                                        	(strncmp(prevChn, input_data_hdr->channel, 3) != 0) || 
                                                        	(strncmp(prevNet, input_data_hdr->network, 2) != 0) ||
								(strncmp(prevLoc, input_data_hdr->location, 2) != 0)) 
								LRECL = get_LRECL(input_data_hdr, lrecord_ptr);
								process_data ();
						} else {
							process_data ();
						...
						strncpy(prevStn, input_data_hdr->station, 5);
						prevStn[5] = 0;

                                              	strncpy(prevChn, input_data_hdr->channel, 3);
						prevChn[3] = 0;

						strncpy(prevNet, input_data_hdr->network, 2);
						prevNet[2] = 0;

 						strncpy(prevLoc, input_data_hdr->location, 2);
						prevNet[2] = 0;
	...
	{
		if (snoop_flag)
			fprintf(stderr, "Print Abbreviations\n");
		print_volh ();
		print_abrvd ();
	}
...
/* -------------------------------------------------------------------- */
int process_data_rec()

{

	struct data_blk_1000 *p;

	struct input_data_hdr *hdr = (struct input_data_hdr *)(lrecord_ptr + 8);
	int default_Lrecl;
	...
	LRECL = get_stn_chn_Lrecl(hdr->station,
                                  hdr->channel,
                                  type10.version >= 2.3 ?
                                                hdr->network:
                                                "",
	...
	if (LRECL == 0 || (LRECL % 256 != 0))
        ...
        if (hdr->bofb != 0) 
                p = scan_for_blk_1000(lrecord_ptr + hdr->bofb,
                                        lrecord_ptr);
        else
                p = NULL;
        ...
        if (p)
                ...
                LRECL = scan_for_lrecl(lrecord_ptr + 8);
...
/* -------------------------------------------------------------------- */
struct data_blk_2000 *parse_type_2k();

int process_blk_2k(struct input_data_hdr *hdr, char *buff)

{
	int err;

	struct data_blk_2000 *b_2k;

	b_2k = parse_type_2k(buff);

	err =  output_b2k(hdr, b_2k);
...

{
	...
	for (i = 0; i < type12.number_spans; i++)
	{
		...
		seek_pos = ((type12.timespan[i].sequence_number - 1) * LRECL) - ftell(inputfile) + Volume_start;

		
		fseek(inputfile, seek_pos, 1);
...

{
	...
	char wrkstr[6];
        ...
        {
		strcpy(wrkstr, type11.station[i].station_id);
	        ...
	        if (!chk_station(wrkstr))
				...
				ftell(inputfile) + Volume_start;

                fseek(inputfile, seek_pos, 1);

		process_station_lrecs(inputfile, i);
...
void where_am_i(char *lrecord, FILE *file) {
	if (inputfile != NULL) 
		printf("fpos = %d ", ftell(file));
	if (lrecord != NULL) 
		printf("lrec = %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c",
			lrecord[0],
			lrecord[1],
			lrecord[2],
			lrecord[3],
...
void position_to_record_boundary(int *num_bytes_read, char *precord, char *lrecord, FILE *inputfile) {
	...
	if (*num_bytes_read != i && !feof(inputfile)) {
		fprintf(stderr,  "Repositioning back %d bytes from %d to be on an even record boundary.\n", 
...

{
	...
	do {
		...
		if (lrecord_ptr[6] == 'D' || lrecord_ptr[6] == 'Q' || lrecord_ptr[6] == 'R') {
...

{
	...
	case 'e':
		fprintf(stderr,"\te = extract event/station data\n");
               	...
               	break;
	case 'E':
		fprintf(stderr,"\tE = output filename includes endtime\n");
                output_endtime = TRUE;
                break;
	...
	if (stat(output_dir, &output_stat)) {
		fprintf(stderr,"Output dir %s not found.\n", output_dir); 
		exit (1);
	} 
	/* verify output_dir is dir */
	if (!S_ISDIR(output_stat.st_mode)) {
		fprintf(stderr,"Output dir %s not a directory.\n", output_dir);
		exit (1);
	}
...
char *filenames;
{
	...
	else
	{
		...
		if (alt_headerfile == NULL)
		{
			fprintf(stderr, "WARNING - unable to open alternate header file\n");
		}
	}
...

{
	...
	/*num_bytes = read(fileno(fptr), precord, PRECL);*/
   	...
   	if (num_bytes == 0)
	{
		fprintf(stderr, "Unable to \"read\" the seed volume!\n");
