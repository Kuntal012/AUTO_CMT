...
struct type48 *find_type_48_stage_0();
...

{
	...
	A0 = get_A0(&poles, &zeros, 
			...
			sizeof(hed->station.cal)/sizeof(hed->station.cal[0]));
...

{
	...
	*zeros = NULL;
	...
	while (r != NULL) {
		if (r->type == 'P') {
			...
			if ((*zeros = (struct type53sub *)
				realloc(*zeros, sizeof(struct type53sub) * 
						((type_53->stage == 1 ? type_53->number_zeroes + gammas : 
		...
		/* type 53s */
			...
			if ((stage == 1 ? type_43->number_zeroes + gammas : 
				...
				if ((*zeros = (struct type53sub *)
					realloc(*zeros, sizeof(struct type53sub) * 
						(stage == 1 ? type_43->number_zeroes + gammas :
	...
	{
		type_48 = find_type_48_stage_0(res_head);
