...
%% Load Stations list
sta=importdata('Stations_PF.txt');
...
lat_n=36.2;       % Latitude North
lon_w=-120.6;     % Longitude West
...
%                         end
        ...
        for i=1:length(id_ev)
            ...
            c(cc_ev)=extractBefore(s(i,1),3);
            s(cc_ev,1)=extractAfter(s(cc_ev,1),2);
            cc_ev=cc_ev+1;
            
        end
         UF=[UF(id_ev,1) c' s(:,1:3) UF(id_ev,3:9)];
         U=U(id_ev,:);
      ...
      for rit=1:rw 
         ...
         dep=str2double(extractBefore(Best_res_txt{rit,12},3));
         d_range=dep-6:2:dep+6;
          % Creating folder for storing paramer files
        par_folNam=strcat(num2str(year),num2str(month),num2str(start_day),'_',num2str(start_hour),'_',num2str(end_hour));
%         comEval(strcat('mkdir Param_AD/',par_folNam))
        eval(strcat("cd ",parent_Dir,'Param_AD/',par_folNam));
